...
Change the symbol cycling key to ';'. Note that this doesn't add a prefix, we do that later with aas to ";s"
#+BEGIN_SRC emacs-lisp
(after! cdlatex
  (setq cdlatex-math-symbol-prefix ?\;) ;; this doesn't add the cdlatex-math-symbol keybind
#+END_SRC
