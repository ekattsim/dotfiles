...
This currently makes the included content look much more package-like than it truly is.
...
Applying lexical binding to the config file is good for a number of reasons, among which it's (slightly) faster than dynamic binding (see [[https://nullprogram.com/blog/2016/12/22/][this blog post]] for more info).
...
Within ~confpkg-tangle-finalize~ we carefully order each step so that the most important steps go first, to minimise the impact should a particular step fail.
...
(python +lsp +tree-sitter +conda +uv)            ; beautiful is better than ugly
...
Let's enable defer by default because it looks like a naked use-package call loads the entire package as opposed to just the *-autoload.el file.
...
(package! almost-mono-themes
  :recipe (:type git
           ...
           :files ("almost-mono-themes.el")))
#+end_src

***** IDEA [#A] Create a custom monochrome theme
...
Add Roboto Mono and ET(Edward Tufte) Book as fixed and variable pitch fonts respectively.

#+BEGIN_SRC emacs-lisp
(when (member "Roboto Mono" (font-family-list))
  (setq doom-font (font-spec :family "Roboto Mono" :size 17.5)))

(when (member "ETBembo" (font-family-list))
  ;; variable pitch font will be a little larger than fixed fonts
  (setq doom-variable-pitch-font (font-spec :family "ETBembo" :size 21.0)))

(doom/reload-font)
#+END_SRC

Enable =mixed-pitch-mode= for all text mode derivations.
...
Wrap =mixed-pitch-mode= with advice so that it doesn't set the =mixed-pitch-fixed-pitch-faces=' =:weight= attribute.

#+begin_src emacs-lisp
(defun my-mixed-pitch--family-only (orig &optional ARG)
  (let ((old (face-attribute 'default :weight)))
    (set-face-attribute 'default nil :weight 'unspecified)
    (funcall orig ARG)
    (set-face-attribute 'default nil :weight old)))

(advice-add 'mixed-pitch-mode :around #'my-mixed-pitch--family-only))
#+end_src

**** IDEA Check out [[https://input.djr.com/info/][Input]]

*** Modeline
#+call: confpkg(name="nano-modeline", via="none")
...
(defun nano-modeline-unload-function ()
  ...
  (setq-default mode-line-format (custom-reevaluate-setting 'mode-line-format))
...
Configure gpg to loopback to emacs for prompts.

#+begin_src emacs-lisp
(setq epg-pinentry-mode 'loopback)

(defvar-local my/gpg-encrypted nil)

(defun my/gpg-decrypt-if-needed ()
  (when (save-excursion
          (goto-char (point-min))
          (looking-at "-----BEGIN PGP MESSAGE-----"))
    (epa-decrypt-region (point-min) (point-max))
    (setq my/gpg-encrypted t)))

(defun my/gpg-encrypt-before-save ()
  (when my/gpg-encrypted
    (epa-encrypt-region (point-min) (point-max) nil nil nil)
    (setq my/gpg-encrypted nil)))

;; (add-hook 'find-file-hook #'my/gpg-decrypt-if-needed)
;; (add-hook 'before-save-hook #'my/gpg-encrypt-before-save)

;; (remove-hook 'find-file-hook #'my/gpg-decrypt-if-needed)
;; (remove-hook 'before-save-hook #'my/gpg-encrypt-before-save)
#+end_src
...
Whitespace mode visualizes tabs instead of them keeping them invisible. Let's disable this globally by default.
...
I want to start using "/" and "?" instead of snipe, and I want highlights to end after navigation.
...
This is weird and idk why but ~M-<return>~ gets in the way of transient mapped binds.

#+begin_src emacs-lisp
(map! :map org-mode-map
      :n "M-<return>" nil
      :n "M-RET" #'org-ctrl-c-ret)
#+end_src
...
Set title and heading font sizes. And also temporarily make headings inherit variable-pitch face to keep headings variable pitched in the ~almost-mono-black~ theme.
...
(dolist (face '((org-level-1 . 1.35)
                ...
                (org-level-8 . 1.1)))
  (let* ((f (car face))
         (current-inherit (face-attribute f :inherit))
         (new-inherit (if (listp current-inherit)
                          (if (eq (car current-inherit) 'variable-pitch)
                              current-inherit
                            (cons 'variable-pitch current-inherit))
                        (if (eq current-inherit 'variable-pitch)
                            'variable-pitch
                          (list 'variable-pitch current-inherit)))))
...
Configure org-latex-preview to load after org which reveals source text under the fragment being pointed to.
...
(setq org-latex-preview-mode-update-delay 0)  ;; preview instantly

;; svg is more convenient because it zoom and theme invariant
(setq org-latex-preview-process-default 'dvisvgm)

;; don't cache previews across sessions to save space
...
Auto preview links after babel execution. This is useful because links are inserted as results of execution.

#+begin_src emacs-lisp
(defun my/org-link-preview-refresh-after-babel ()
  (run-at-time
   0.1 nil
   'org-link-preview-refresh))

(add-hook 'org-babel-after-execute-hook
          #'my/org-link-preview-refresh-after-babel)
#+end_src
...
Use latexmk to build LaTeX so I don't need to deal with multi-compilation headaches.
...
Let's standardize on better defaults than ox-latex

#+begin_src emacs-lisp
(setq org-export-with-toc nil
      org-export-with-section-numbers nil)

(add-to-list 'org-latex-packages-alist '("margin=1in" "geometry" nil nil))
#+end_src
...
(setq vhdl-ext-project-alist
      '(("single-cycle-rv32i"
         ...
         :files ("src/*.vhd"
                 "sim/*.vhd")
         :ignore-files ("src/InstMem.vhd")
         :workdir "library/work"
         )
        ("rv32i"
         :root "~/org/projects/computer-architecture/rv32i"
         :files ("core/*.vhd")
         :workdir "library/work"
         ))
      )
...
I want to work on my blog using only emacs and firefox as opposed to npm dev packages. We can use the firefox extension [[https://github.com/blaise-io/live-reload#readme][Live Reload]] to take care of the auto reloading on file changes. We still need a web-server which serves our html on localhost, that's where =simple-httpd= comes in.
...
Let's also configure the typst compilation buffer to display at the bottom as opposed to messing with our preview buffer.
...
Open new pdf buffers in themed mode if system appearance is dark.
...
Enable/Disable themed mode for all existing pdf buffers on the change of system appearance.

#+begin_src emacs-lisp
(defun my/sync-pdf-theme-with-system (appearance)
  "Toggle `pdf-view-themed-minor-mode` based on system APPEARANCE."
  (let ((enable-p (eq appearance 'dark)))
    ;; Iterate over all existing buffers
    (dolist (buf (buffer-list))
      (with-current-buffer buf
        (when (derived-mode-p 'pdf-view-mode)
          (pdf-view-themed-minor-mode (if enable-p 1 -1)))))))

(add-hook 'ns-system-appearance-change-functions
          #'my/sync-pdf-theme-with-system
          t)
#+end_src
...
#+begin_src emacs-lisp
(after! ob-jupyter
  (setq my/jupyter-kernel-connection-file "/ssh:gpu-server:/home/asurakan/.local/share/jupyter/runtime/kernel-85a52649-77a9-437a-a8f8-799fb74a48dc.json")
  (setq org-babel-default-header-args:jupyter-python
        `((:results . "output scalar replace")
          (:session . ,my/jupyter-kernel-connection-file)
          (:kernel . "python3")
          (:async . "yes")
          (:exports . "both"))))
#+end_src

*** Dynamic Kernelspec Generation
#+call: confpkg(name="uv-jupyter-kernels")
...
(after! ob-jupyter
  ...
  (defgroup uv-jupyter-kernels nil
    "Dynamic uv-backed kernelspec generation for ob-jupyter."
    :group 'jupyter
    :prefix "uv-jupyter-kernels-")

  (defcustom uv-jupyter-kernels-directory
    (expand-file-name "~/.local/share/uv/tools/jupyter-core/share/jupyter/kernels/")
    "Directory where generated kernelspecs are written."
    :type 'directory)

  (defcustom uv-jupyter-command "uv"
    "The uv executable used in generated kernelspecs."
    :type 'string)

  (defcustom uv-jupyter-python-command "python"
    "The Python command passed to `uv run`."
    :type 'string)

  (defcustom uv-jupyter-default-packages '()
    "Packages always added to generated uv kernels, besides ipykernel."
    :type '(repeat string))

  (defcustom uv-jupyter-kernel-prefix "uv"
    "Prefix used for generated kernel names."
    :type 'string)

  (defun uv-jupyter--split-package-string (s)
    (when (stringp s)
      (split-string s "," t "[[:space:]\n\t\r]+")))

  (defun uv-jupyter--normalize-packages (packages)
    (sort
     (delete-dups
      (cl-remove-if #'string-empty-p
                    (mapcar #'string-trim packages)))
     #'string-lessp))

  (defun uv-jupyter--safe-component (s)
    (let ((s (downcase s)))
  ...
  (defun uv-jupyter--package-list-p (kernel)
    "Treat KERNEL as a package list iff it contains commas.
This avoids stealing normal kernelspec names like \"python3\"."
    (and (stringp kernel)
         (string-match-p "," kernel)))

  (defun uv-jupyter--packages-from-kernel-header (kernel)
    (uv-jupyter--normalize-packages
     (append uv-jupyter-default-packages
             (uv-jupyter--split-package-string kernel))))

  (defun uv-jupyter--kernel-name (packages)
    (let ((suffix (mapconcat #'uv-jupyter--safe-component packages "-")))
      (if (string-empty-p suffix)
          (format "%s-python" uv-jupyter-kernel-prefix)
        (format "%s-%s" uv-jupyter-kernel-prefix suffix))))

  (defun uv-jupyter--kernel-display-name (packages)
    (format "Python (uv: %s)"
            (if packages
                (string-join packages ",")
              "base")))

  (defun uv-jupyter--kernel-dir (kernel-name)
    (expand-file-name kernel-name uv-jupyter-kernels-directory))

  (defun uv-jupyter--kernel-json-path (kernel-name)
    (expand-file-name "kernel.json" (uv-jupyter--kernel-dir kernel-name)))

  (defun uv-jupyter--argv (packages)
    (append
     (list uv-jupyter-command "run")
     (cl-mapcan (lambda (pkg) (list "--with" pkg))
                (append '("ipykernel") packages))
     (list uv-jupyter-python-command
           "-m" "ipykernel_launcher"
           "-f" "{connection_file}")))

  (defun uv-jupyter--kernel-plist (packages)
    `((argv . ,(uv-jupyter--argv packages))
      (display_name . ,(uv-jupyter--kernel-display-name packages))
      (language . "python")
      (metadata . ((debugger . t)))))

  (defun uv-jupyter--json-encode-kernel (packages)
    (let ((json-object-type 'alist)
          (json-array-type 'list)
          (json-encoding-pretty-print t))
      (json-encode (uv-jupyter--kernel-plist packages))))

  (defun uv-jupyter--ensure-directory (dir)
    (unless (file-directory-p dir)
      (make-directory dir t)))

  (defun uv-jupyter--write-file-if-changed (file content)
    (let ((current (when (file-exists-p file)
                     (with-temp-buffer
                       (insert-file-contents file)
                       (buffer-string)))))
  ...
  (defun uv-jupyter-ensure-kernelspec-for-packages (packages)
    "Ensure a uv-backed kernelspec exists for PACKAGES.
Return the generated kernelspec name."
    (let* ((packages (uv-jupyter--normalize-packages packages))
           (kernel-name (uv-jupyter--kernel-name packages))
           (kernel-dir (uv-jupyter--kernel-dir kernel-name))
           (kernel-json (uv-jupyter--kernel-json-path kernel-name))
           (json-content (uv-jupyter--json-encode-kernel packages)))
      (uv-jupyter--ensure-directory kernel-dir)
      (uv-jupyter--write-file-if-changed kernel-json json-content)
      (ignore-errors (jupyter-refresh-kernelspecs))
      kernel-name))

  (defun uv-jupyter-resolve-kernel-header (kernel)
    "If KERNEL looks like a package list, generate a kernelspec and return its name.
Otherwise return KERNEL unchanged."
    (if (uv-jupyter--package-list-p kernel)
        (uv-jupyter-ensure-kernelspec-for-packages
         (uv-jupyter--packages-from-kernel-header kernel))
      kernel))

  (defun uv-jupyter--rewrite-jupyter-params (params)
    (let* ((kernel (alist-get :kernel params))
           (resolved (uv-jupyter-resolve-kernel-header kernel)))
      (if (equal kernel resolved)
          params
        (cons (cons :kernel resolved)
              (assq-delete-all :kernel params)))))

  (defun uv-jupyter--around-org-babel-execute-jupyter (orig-fn body params)
    (funcall orig-fn body (uv-jupyter--rewrite-jupyter-params params)))

;;;###autoload
  (defun uv-jupyter-enable ()
    "Enable dynamic uv-backed kernelspec generation for ob-jupyter."
    (interactive)
    (advice-add 'org-babel-execute:jupyter :around
                #'uv-jupyter--around-org-babel-execute-jupyter))

;;;###autoload
  (defun uv-jupyter-disable ()
    "Disable dynamic uv-backed kernelspec generation for ob-jupyter."
    (interactive)
    (advice-remove 'org-babel-execute:jupyter
                   #'uv-jupyter--around-org-babel-execute-jupyter))

  (provide 'uv-jupyter-kernels)
  (uv-jupyter-enable))
#+end_src
...
** doom-scratch
:PROPERTIES:
:ID:       9b53dabd-fb47-4ad1-bbc0-cbe364b033dd
:END:
#+call: confpkg()

Start doom scratch buffers in org mode.
...
Rendering performance for pngs improves if we use a native decoder instead of imagemagick.
...
Doom is doing too much with the node template string, configure a more minimal display that just shows the title.

#+begin_src emacs-lisp
(use-package! org-roam
  :config
  (setq org-roam-node-display-template
        (concat "${title:*} " "${doom-tags:20}"))
#+end_src

Adjust the default template to allow insertion of file tags as well.
...
Fix ox-html's poor handling of id links
...
**** Deft
:PROPERTIES:
:ID:       206067c8-9d36-4ad0-b397-2f9681803b9c
:END:
...
(use-package! elfeed
  ...
  (setq elfeed-feeds
        '("https://xkcd.com/rss.xml"
          ...
          ("https://old.reddit.com/r/hardware.rss" hardware)
          ("https://old.reddit.com/r/computerarchitecture.rss" comparch)
          ("https://old.reddit.com/r/chipdesign.rss" comparch)
          "https://japanrunningnews.blogspot.com/rss.xml"
          ("https://semiwiki.com/feed" semiconductors)
          "https://owllife.kennesaw.edu/organization/isa/events.rss"
          ("https://mattrickard.com/rss" AI)
          ("https://semianalysis.com/rss" semiconductors)
          ("https://geohot.github.io/blog/feed.xml" culture)
          ("https://stratechery.passport.online/feed/rss/CaAPdzwgcYLX9nCkfGBAD" business technology)
          ))
#+end_src

Block execution of elfeed if currently at work.

#+begin_src emacs-lisp
(defun my/block-elfeed-at-work-advice (orig-fun &rest args)
  (unless (and (boundp 'my/query-pomm-state)
               (my/query-pomm-state 'running 'work))
    (apply orig-fun args)))

(advice-add 'elfeed :around 'my/block-elfeed-at-work-advice)
#+end_src
...
Emacs is the perfect application for an AI to interact with.

*** agent-shell (code agents)

This is kind of for vibe coding.
...
(use-package! agent-shell
  ...
  (require 'acp)
  (map! :map override
        :nvi "M-<tab>" #'agent-shell)
  :config
#+end_src

Configure codex to be the default agent
...
We can make the agent interface play nice with evil mode now that we're out of the terminal.
...
*** [[https://github.com/karthink/gptel][gptel (LLM chat API)]]
:PROPERTIES:
:ID:       45f18400-eb8b-4d37-b943-75a7240f516e
:END:

This is just for regular LLM use.
...
(use-package! gptel
  ...
  :config
  (setq epg-pinentry-mode 'loopback)
  (setq gptel-backend (gptel-make-openai-chatgpt "ChatGPT-Codex")
        gptel-model 'gpt-5.4)
#+end_src
...
Let's create a general preset for one off queries.

#+begin_src emacs-lisp
(require 'gptel-agent)
(gptel-make-preset 'general
    :description "A preset for general queries."
    :backend "ChatGPT-Codex"
    :model 'gpt-5.4
    :system "You are a large language model and your job is to be a helpful assistant. Respond concisely and only bring up information that is directly requested by the prompt. This means no 'additional information' or 'also relevant' information should populate the response. Use the available tools if your knowledge is lacking or if explicitly directed by the user."
    :tools '("WebSearch" "WebFetch" "YouTube"))
#+end_src

Close use-package paren
...
(use-package! gptel-agent
  :init
  (map! :leader :desc "Portable agentic intelligence"
        "j a" #'gptel-agent)
...
For quick summarization, we can use gptel-quick.

#+begin_src emacs-lisp
(use-package! gptel-quick
  :init
  (map! :leader :desc "Quick Summaries"
        "?" #'gptel-quick)
  :config
  (setq gptel-quick-use-context t
        gptel-quick-timeout 99))
#+end_src

*** NEXT [#C] [[https://github.com/milanglacier/minuet-ai.el][minuet (LLM FIM API)]]

This is for code autocompletion.
...
Xwidgets are implemented as subclassed [[https://developer.apple.com/documentation/appkit/nsview][NSViews]] under the emacs parent view, running the system WebKit engine within this child view.
...
(use-package! xwwp
  ...
  (map! :leader
        "o b" #'xwwp
        "o B" #'xwwp-other-window)
  :config
  (map! :map xwidget-webkit-mode-map
        :n "f" #'xwwp-follow-link)
  (after! evil-collection-xwidget
    (map! :leader
...
(defun my/create-org-project ()
  ...
  (let* ((project-name (read-string "Project name: "))
    ...
    (if (file-exists-p project-dir)
      ...
      (write-region (format-message "#+title: %s"
...
Now let's add a rename operation on projects.

#+begin_src emacs-lisp
(defun my/rename-org-project ()
  "Rename an Org project using `org-project-files`."
  (interactive)
  (let* ((old-file (completing-read
                    "Rename project: "
                    org-project-files
                    nil t))
         (old-file (expand-file-name old-file))
         (old-name (file-name-base old-file))
         (new-name (read-string "New project name: "))
         (old-dir (file-name-directory old-file))
         (has-own-dir
          (not (file-equal-p
                (expand-file-name org-project-directory)
                (directory-file-name old-dir))))
         (new-dir (if has-own-dir
                      (expand-file-name new-name org-project-directory)
                    old-dir))
         (new-file (expand-file-name (concat new-name ".org") new-dir)))
    (when (and has-own-dir (file-exists-p new-dir))
      (user-error "Project '%s' already exists" new-name))
    (when (file-exists-p new-file)
      (user-error "File '%s' already exists" new-file))

    (when has-own-dir
      (rename-file (directory-file-name old-dir) new-dir)
      (projectile-remove-known-project old-dir)
      (projectile-add-known-project new-dir))

    (let ((source-file
           (if has-own-dir
               (expand-file-name (concat old-name ".org") new-dir)
             old-file)))
    ...
    (with-temp-buffer
      ...
      (write-region nil nil new-file))

    (find-file new-file)
    (message "Renamed project to: %s" new-name)))
#+end_src

Let's add a file-watch that rebuilds =org-project-files= and =org-agenda-files= on any changes in the =org-project-directory= (nonrecursive).
...
Doom sets =projectile-ignored-project-function= to ~doom-project-ignored-p~ to decide whether a project can be added as "known". Let's remove this limitation.
...
**** DONE interactive archive
SCHEDULED: <2025-08-18 Mon>
:PROPERTIES:
:ID:       52c26fea-8046-4b2f-b9f2-a16f81a34d04
:END:
...
Shorten deadline reminders to 2d by default. 2 weeks prior warnings is too much org. And also don't show deadline warnings if the task is scheduled.
...
#+BEGIN_SRC emacs-lisp
(map! :map override-global-map
      "C-t" #'+workspace/switch-to
      [C-tab] #'+workspace/other)
#+END_SRC
...
We have to provide the tufte and ox-tufte stylesheets.

#+begin_src emacs-lisp
(setq org-html-head "<link rel=\"stylesheet\" href=\"/Users/abhijeetsurakanti/dotfiles/emacs/dot-config/doom/misc/tufte.css\">
<link rel=\"stylesheet\" href=\"/Users/abhijeetsurakanti/dotfiles/emacs/dot-config/emacs/.local/straight/repos/ox-tufte/src/ox-tufte.css\">")
#+end_src
...
(use-package! pomm
  ...
  (setq pomm-csv-history-file (concat org-directory
                                      "/pomm-history.csv"))
  (pomm-mode-line-mode)
#+end_src

And for integration with =org-clock=.
...
Tweak work/break times.

#+begin_src emacs-lisp
  (setq pomm-work-period 50
        pomm-short-break-period 10
        pomm-number-of-periods 99)
#+end_src

Kill any elfeed buffers at the end of a break.

#+begin_src emacs-lisp
(defun my/pomm-kill-elfeed ()
  (when (my/query-pomm-state 'running 'work)
              (dolist (buf (buffer-list))
                (when (string-match "elfeed"
                                    (buffer-name buf))
                  (kill-buffer buf)))))

(add-hook 'pomm-on-status-changed-hook
          'my/pomm-kill-elfeed)
#+end_src

+I want =pomm-ask-before-work= to be enabled only after a long break.+ This is irrelevant now since I've disabled long breaks.

#+begin_src emacs-lisp
(defun my/pomm-ask-after-long-break ()
  (if (my/query-pomm-state 'running 'long-break)
                (setq pomm-ask-before-work t)
              (setq pomm-ask-before-work nil)))

(add-hook 'pomm-on-status-changed-hook
          'my/pomm-ask-after-long-break)
#+end_src
...
Define a util function to query =pomm--state= easily.

#+begin_src emacs-lisp
(defun my/query-pomm-state (status type)
  (let* ((timer-status (cdr (assoc 'status
                                   pomm--state)))
         (curr-timer (cdr (assoc 'current
                                 pomm--state)))
         (timer-type (cdr (assoc 'kind
                                 curr-timer))))
    (and (eq timer-status status)
         (eq timer-type type))))
#+end_src
