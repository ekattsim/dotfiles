...
(setq org-directory "~/zettelkasten/") ;; default location for org files (needs to be set before org loads)
#+END_SRC
...
(defun my/apply-theme (appearance)
  ...
  (load-file "~/.config/doom/subconf/config-org-aesthetics.el"))
...
(use-package! laas
  ...
  (aas-set-snippets 'laas-mode
    ...
    ;; symbols, these are mostly handled by laas
    ";s" (lambda () (interactive) (cdlatex-math-symbol)) ;; symbols prefix

    ;; commands(copy from karthink). We double escape '\' if we're preceding escapable chars
    "mk" '(yas "\\\\( $0 \\\\)") ;; We're trying to pass the string "\\( $0 \\)"
    "dm" '(yas "\\[ $0 \\]")
    ...
    ";v" '(yas "\\vec{$0}")
    ";q" '(yas "\\sqrt{$0}")
    ";(" '(yas "\\left( $0 \\right") ;; note that the right paren is not there(it autoexpands)
    ";[" '(yas "\\left[ $0 \\right")
    ";|" '(yas "\\left| $0 \\right|")
    ))
#+END_SRC
...
*** org-agenda
 #+begin_src emacs-lisp
(setq org-agenda-files '("~/dotfiles/emacs/dot-config/doom/literate-config/config.org" "~/zettelkasten/"))
 #+end_src
