...
#+begin_src emacs-lisp
(setq org-todo-keywords '((sequence "INBOX(I)" "TODO(t)" "NEXT(n)" "WAIT(w)" "IDEA(i)" "|" "DONE(d)" "KILL(k)")))
...
In this workflow all TODOs have to be scheduled, so let's an advice over the =org-todo= and =org-agenda-todo= functions.
#+begin_src emacs-lisp
(defadvice! schedule-todo (_arg)
